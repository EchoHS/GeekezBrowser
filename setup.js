...
const xrayAssets = require('./src/main/xray-assets');
...
function getPlatformInfo() {
    ...
    const arch = os.arch();
    let exeName = 'xray';
    const xrayAsset = xrayAssets.resolveXrayAssetName({
        platform,
        arch
    });
    if (!xrayAsset) throw new Error(`Unsupported Platform/Arch: ${[platform, arch].join('-')}`);
    if (platform === 'win32') {
        exeName = 'xray.exe';
    }
